...
class ControllerNewsBlogArticle extends Controller {
	...
	public function index() {
		...
		$images_size_articles_big = array(734, 412);
		...
		if ($article_info) {
			...
			if ($article_info['image']) {
				...
				$data['thumb'] 		= '';
					// $this->model_tool_image->resize($article_info['image'], $images_size_articles_small[0], $images_size_articles_small[1]);
			} else {
				$data['original'] 	= false;
				$data['popup'] 		= false;
				$data['thumb'] 		= false;
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'popup' 	=> '',
						// $this->model_tool_image->resize($result['image'], $images_size_articles_big[0], $images_size_articles_big[1]),
					'thumb' 	=> '',
						// $this->model_tool_image->resize($result['image'], $images_size_articles_small[0], $images_size_articles_small[1])
				);
			}
            ...
            foreach ($results as $result) {
				if ($result['image']) {
					$image = '';
						// $this->model_tool_image->resize($result['image'], $this->config->get($this->config->get('config_theme') . '_image_related_width'), $this->config->get($this->config->get('config_theme') . '_image_related_height'));
				} else {
					$image = '';
						// $this->model_tool_image->resize('placeholder.png', $this->config->get($this->config->get('config_theme') . '_image_related_width'), $this->config->get($this->config->get('config_theme') . '_image_related_height'));
				}
