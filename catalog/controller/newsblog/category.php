		...
		if ($category_info) {
         ...
         $articles_image_size = array(372, 245);
			...
			if ($category_info['image']) {
				...
				$data['thumb'] 		= '';
				// $this->model_tool_image->resize($category_info['image'], $category_image_size[0], $category_image_size[1]);
			} else {
				$data['original'] 	= '';
				$data['thumb'] 		= '';
			}
			...
			foreach ($categories as $category) {
				if ($category['image']) {
					...
					$thumb 		= '';
					// $this->model_tool_image->resize($category['image'], $articles_image_size[0], $articles_image_size[1]);
				} else {
					$original 	= false;
					$thumb 		= false;
				}
